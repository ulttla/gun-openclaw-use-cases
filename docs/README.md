# Documentation map

This folder contains public-safe lab notes and case studies.

## Main notes

- [AI Engineering Lab](ai-engineering-lab.md)
- [AI Engineering Control Plane](ai-assisted-engineering-control-plane.md)
- [Long Work Window Playbook](long-work-window-playbook.md)
- [AzVision App Development](azvision-network-path-analysis.md)
- [Gun-Wiki Brain](gun-wiki-brain.md)
- [Operator Resilience and Update Safety](operator-resilience-and-update-safety.md)
- [Hermes Complementary Operator Lane](hermes-secondary-operator-lane.md)
- [Restart Continuity Guard](restart-continuity-guard.md)
- [Isolated Update Worktree Pattern](isolated-update-worktree.md)
- [Research Lane Web Setup](research-lane-web-setup.md)
- [Publication Boundary for Marketplace Workflows](publication-boundary-marketplace.md)
- [Spark Local LLM Lab](spark-local-llm-lab.md)
- [Security and Sanitization](security-and-sanitization.md)
- [Public Release Evidence Pattern](public-release-evidence.md)
- [Communicating AI-Assisted Work](communicating-ai-assisted-work.md)

## Decisions

- [Single orchestrator, multiple specialist lanes](adr/0001-single-orchestrator.md)
- [Consensus evidence before closeout](adr/0002-consensus-before-closeout.md)
- [Sanitized case-study repository instead of raw setup dump](adr/0003-sanitized-case-study-repo.md)

## Examples and evidence templates

- [Harness packet template](../examples/harness-packet-template.md)
- [Long Work Window closeout template](../examples/lww-closeout-template.md)
- [Disagreement to closeout example](../examples/disagreement-to-closeout.md)
- [Sanitized path evidence](../examples/sanitized-path-evidence.json)
- [Synthetic Azure path sample](../examples/synthetic-azure-path-sample.json)

## What is intentionally not here

Internal deployment plans, approval packets, private review notes, and draft social posts are kept out of the public tree. They are useful working notes, but they are not part of the public lab-notes surface.

## Model-neutral context notes

- [Gun-Wiki Brain](gun-wiki-brain.md) — wiki-first hybrid project memory and LLM-readable context pattern
- [Spark Local LLM Lab](spark-local-llm-lab.md) — local LLM / NVIDIA experimentation boundary

## Korean starter notes

- [Korean notes index](ko/README.md)
- [Korean AI Engineering Lab overview](ko/ai-engineering-lab.md)
- [Korean Long Work Window overview](ko/long-work-window.md)
- [Korean Control Plane overview](ko/control-plane.md)
- [Korean AzVision App Development overview](ko/azvision-network-path-analysis.md)
- [Korean Gun-Wiki Brain overview](ko/gun-wiki-brain.md)
- [Korean Spark Local LLM Lab overview](ko/spark-local-llm-lab.md)
- [Korean Operator Resilience and Update Safety overview](ko/operator-resilience-and-update-safety.md)
- [Korean Hermes Complementary Operator Lane overview](ko/hermes-secondary-operator-lane.md)
- [Korean Security and Sanitization overview](ko/security-and-sanitization.md)
- [Korean Communicating AI-Assisted Work overview](ko/communicating-ai-assisted-work.md)
